  ...
  <style>
      ...
      * {
      ...
      .bm {
        ...
        background-image: url(images/iStock-1125868202-1.jpeg);
...
<div class="jumbotron jumbotron-fluid">
  ...
  <p class="lead">Select a Menu</p> 
            <hr class="my-4"> 
            <form method="GET" action="vegetarianMealType.php"> 
                <select name="emp" onchange='this.form.submit()'> 
                    ...
                    $sql = "SELECT COUNT(MEALS.NAME),MEALS.MEAL_TYPE
                    FROM MEALS
                    WHERE MEALS.NAME = ANY (SELECT VEGETARIAN_LIST.VEG_MEAL_NAME FROM VEGETARIAN_LIST)
                    GROUP BY MEALS.MEAL_TYPE"; 
                    if ($result = mysqli_query($connection, $sql))  
                    { 
                        // loop through the data 
                        while($row = mysqli_fetch_assoc($result)) 
                        { 
                            echo '<option value="'.$row['MEAL_TYPE'].'">'; 
                            echo $row['MEAL_TYPE'];  
                            echo "</option>"; 
                        } 
                ...
                <table class = "table table-striped">
                    ...
                    <?php            
                        ...
                        $sql = "SELECT MEALS.MEAL_TYPE, VEGETARIAN_LIST.VEG_MEAL_NAME, RECIPES.INGREDIENTS, RECIPES.TIME_TO_MAKE
                        from MEALS
                        JOIN VEGETARIAN_LIST ON VEGETARIAN_LIST.VEG_RE_ID = MEALS.RE_ID
                        JOIN RECIPES ON RECIPES.ID = MEALS.RE_ID
                        WHERE MEALS.MEAL_TYPE = '{$_GET['emp']}'"; 

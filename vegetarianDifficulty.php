  ...
  <style>
      ...
      * {
      ...
      .bm {
        ...
        background-image: url(images/Easy-Vegan-Beginner-Recipes-by-Nutriciously-Featured-Image.jpeg);
...
<div class="jumbotron jumbotron-fluid">
  ...
  <p class="lead">Select a Level</p> 
            <hr class="my-4"> 
            <form method="GET" action="vegetarianDifficulty.php"> 
                <select name="emp" onchange='this.form.submit()'> 
                    ...
                    $sql = "SELECT COUNT(RECIPES.DIFFICULTY), RECIPES.DIFFICULTY
                    FROM RECIPES
                    GROUP BY RECIPES.DIFFICULTY"; 
                    if ($result = mysqli_query($connection, $sql))  
                    { 
                        // loop through the data 
                        while($row = mysqli_fetch_assoc($result)) 
                        { 
                            echo '<option value="'.$row['DIFFICULTY'].'">'; 
                            echo $row['DIFFICULTY'];  
                            echo "</option>"; 
                        } 
                ...
                <table class = "table table-striped">
                    ...
                    <?php            
                        ...
                        $sql = "SELECT VEGETARIAN_LIST.VEG_MEAL_NAME, RECIPES.INGREDIENTS, RECIPES.TIME_TO_MAKE, MEALS.BUGET,RECIPES.DIFFICULTY
                        FROM VEGETARIAN_LIST
                        JOIN MEALS ON MEALS.NAME = VEGETARIAN_LIST.VEG_MEAL_NAME AND MEALS.BUGET < 10
                        JOIN RECIPES ON RECIPES.ID = VEGETARIAN_LIST.VEG_RE_ID AND '{$_GET['emp']}' = RECIPES.DIFFICULTY"; 
                    ...
                    ?> 
                    <tr> 
                        <td class = "T"><?php echo $row['VEG_MEAL_NAME'] ?></td>
                        <td class = "T"><?php echo $row['INGREDIENTS'] ?></td>
                        <td class = "T"><?php echo $row['TIME_TO_MAKE'] ?></td>
                        <td class = "T"><?php echo $row['BUGET'] ?></td>
                        <td class = "T"><?php echo $row['DIFFICULTY'] ?></td>
                    </tr> 
                    <?php 
                            } 
                            // release the memory used by the result set 
